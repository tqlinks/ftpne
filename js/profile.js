...
let capturedImage = null; // Lưu trữ ảnh từ clipboard
...
window.onload = async () => {
    ...
    // 2. Khởi tạo túi đồ linh thú
    renderPetInventory();
    initPetToggleState();

    // 3. Khởi tạo xử lý ảnh dán (Clipboard)
    initClipboardPaste();
    
    // 4. Kiểm tra trạng thái điểm danh
    checkDailyStatus();
};
...
function renderUserInfo() {
    ...
    // Sản lượng & Mục tiêu (Progress Bars)
    setVal('input-kinah', u.kinah || 0);
    setVal('input-ody', u.ody || 0); // Đã đổi tên theo HTML mới của bạn

    updateProgress('kinah', u.kinah, u.kGoal);
    
    // Bật nút Admin nếu có quyền
...
async function handleCheckIn() {
    ...
    try {
        ...
        alert("🎉 +5 FPE! Chăm chỉ quá Linh ơi!");
        window.location.reload();
    } catch (e) {
        alert("Lỗi kết nối điểm danh!");
        btn.disabled = false;
    }
}
...
function renderPetInventory() {
    ...
    if (ownedIds.length === 0) {
        list.innerHTML = `<p class="col-span-full text-[10px] text-gray-400 text-center py-4">Chưa có linh thú đồng hành</p>`;
        return;
    }
    ...
    list.innerHTML = ownedIds.map(id => {
        ...
        if (!pet) return "";
        return `
            <div class="bg-white dark:bg-gray-700 p-2 rounded-xl shadow-sm border dark:border-gray-600 text-center relative group">
                <img src="${pet.img}" class="w-10 h-10 mx-auto">
                <p class="text-[8px] font-bold mt-1 truncate dark:text-gray-200">${pet.name}</p>
            </div>
        `;
    }).join('');
}
    ...
    try {
        await fetch(CONFIG.SCRIPT_URL, {
            ...
            body: JSON.stringify({ 
                ...
                kinah: kinah, 
                ody: ody,
                image: capturedImage // Sẽ lưu vào Sheet nếu bạn đã setup cột ảnh
            })
        });
